...
const { syncPayments, verifyTransaction, finalizeConfirmedPayments, recordPayment } = require('../services/stellarService');
const { SCHOOL_WALLET, ACCEPTED_ASSETS, getExplorerUrl } = require('../config/stellarConfig');
...
async function getStudentPayments(req, res, next) {
  try {
    ...
    const result = payments.map(p => {
      ...
      return { ...obj, explorerUrl: getExplorerUrl(p.txHash) };
    });
    res.json(result);
  } catch (err) {
    next(err);
